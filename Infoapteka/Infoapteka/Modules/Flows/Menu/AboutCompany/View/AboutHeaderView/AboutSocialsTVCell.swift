...
protocol AboutSocialsTVCellDelegate {
    func didTappedOnSocialMedia(_ link: String?)
}

extension AboutSocialsTVCell {
    struct Appearance {
...
class AboutSocialsTVCell: UITableViewCell {
    ...
    private lazy var tableView = AboutSocialMediaTV().then {
        $0.backgroundColor = appearance.backViewColor
        $0._delegate = self
    }
    
    // MARK: Properties
    var delegate: AboutSocialsTVCellDelegate?
    ...
    func setupHeader(_ viewModel: AboutCompanyResultVM,
        ...
        setTitleText(type)
        self.viewModel?.setCellMediaType(type)
        self.viewModel = viewModel
    }
}

extension AboutSocialsTVCell: AboutSocialMediaTVDelegate {
